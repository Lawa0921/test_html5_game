...
class RoomAScene extends Phaser.Scene {
    constructor() {
        super({ key: 'RoomAScene' });
        this.gameState = null;
    ...
    createSceneObjects() {
        ...
        // 床鋪（主要物件）
        const bed = this.createInteractiveObject(650, 300, 200, 120, '床鋪🛏️', 0x8B4513, () => {
            this.showBedMenu();
        });

        // 桌子
        const table = this.createInteractiveObject(200, 250, 120, 80, '桌子', 0xD2691E);

        // 椅子
        const chair = this.createInteractiveObject(180, 320, 50, 50, '椅', 0x8B4513);

        // 衣櫃
        const wardrobe = this.createInteractiveObject(100, 150, 100, 180, '衣櫃', 0x654321, () => {
        ...
        // 茶几
        const teaTable = this.createInteractiveObject(450, 400, 100, 60, '茶几', 0xD2691E);
    ...
    createCharacters() {
        ...
        this.gameState.employees.forEach(employee => {
            if (employee.unlocked && employee.workStatus.assignedStation === 'room-a') {
                const sprite = new CharacterSprite(
                    this,
                    employee,
                    employee.workStatus.position.x || 300,
    ...
    createBottomBar() {
        ...
        // 前往客房B按鈕
        const toRoomBBtn = this.createButton(500, 0, 180, 40, '前往客房B', () => {
            this.sceneManager.toRoomB();
        }, 0x3498DB);

        bar.add([bg, backBtn, cleanBtn, toRoomBBtn]);
        this.bottomBar = bar;
    }
